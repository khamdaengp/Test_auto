...
test.describe('Automated Visual Scenario', () => {
  test('execute visual no-code test flow', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('button').filter({ hasText: 'Search' }).or(page.getByRole('button', { name: 'Search' })).or(page.getByText('Search', { exact: true })).first()).toBeVisible({ timeout: 10_000 });

    // Step 15: Verify text "Reset" is visible
    await expect(page.locator('button').filter({ hasText: 'Reset' }).or(page.getByRole('button', { name: 'Reset' })).or(page.getByText('Reset', { exact: true })).first()).toBeVisible({ timeout: 10_000 });

    // Step 16: Verify text "Create" is visible
    await expect(page.locator('button').filter({ hasText: 'Create' }).or(page.getByRole('button', { name: 'Create' })).or(page.getByText('Create', { exact: true })).first()).toBeVisible({ timeout: 10_000 });

    // Step 17: Verify text "Import files" is visible
    await expect(page.locator('button').filter({ hasText: 'Import files' }).or(page.getByRole('button', { name: 'Import files' })).or(page.getByText('Import files', { exact: true })).first()).toBeVisible({ timeout: 10_000 });

    // Step 18: Verify text "Export files" is visible
    await expect(page.locator('button').filter({ hasText: 'Export files' }).or(page.getByRole('button', { name: 'Export files' })).or(page.getByText('Export files', { exact: true })).first()).toBeVisible({ timeout: 10_000 });
