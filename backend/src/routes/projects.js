...
router.post('/', async (req, res) => {
  try {
    const { name, baseUrl, description = '', env_vars = {} } = req.body;
    if (!name || !baseUrl) {
    ...
    const { rows } = await db.query(
      `INSERT INTO projects (id, name, base_url, description, env_vars)
       VALUES ($1, $2, $3, $4, $5)
       RETURNING *`,
      [id, name.trim(), baseUrl.trim(), description.trim(), typeof env_vars === 'object' ? env_vars : {}]
    );
  ...
  try {
    ...
    const { rows } = await db.query(
      `UPDATE projects
       ...
       WHERE id = $5
       RETURNING *`,
      [name, baseUrl, description, env_vars, req.params.id]
    );
...
/**
 * PUT /api/projects/:id/env - Update environment variables for a specific project
 */
  ...
  try {
    ...
    const { rows } = await db.query(
      `UPDATE projects
       ...
       WHERE id = $2
       RETURNING *`,
      [typeof env_vars === 'object' && env_vars !== null ? env_vars : {}, req.params.id]
    );
